...
SCRAPER_URL="${SCRAPER_URL:-http://159.69.150.218:8080}"
...
cat > "$IPC_DIR/enrich_$(date +%s).json" << TASK1
{
  ...
  "prompt": "You are running the daily jojo enrichment task. Your job:\n\n1. Read the scraper feed data provided below. These are fresh internship postings (≤72h old) from the curated top-50 direct-source companies (Greenhouse, Ashby, Lever, Amazon, Apple, Workday). Only Summer/Fall/Spring 2026 roles that pass the seasonal location rules are included.\n\n2. For each company with new postings, update or create the company profile at /workspace/extra/vault/companies/{company-slug}.md with:\n   - Company overview (what they do, size, culture)\n   - Open intern roles from the feed (title, season, location, apply link)\n   - Interview format (your knowledge + LeetCode patterns)\n   - Common interview questions\n   - Salary/compensation data\n   - Application tips\n\n3. Update /workspace/extra/vault/intel/interview-questions.md with any new company-specific questions.\n\n4. Update /workspace/extra/vault/intel/market-trends.md with notable trends from the direct-source companies only (new roles opening, application deadlines, etc.)\n\n5. Save the current timestamp to /workspace/group/last_enrichment_ts for next run.\n\nBe thorough but concise. Write in a helpful, factual tone. Include specific details that would help Ruthwik prepare for applications and interviews at these top-tier direct-hire companies.",
...
echo "   4. UIUC scout queue lives in Vault/NanoClaw/uiuc-scout via scraper sync"
echo ""
